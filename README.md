...
A modular, deterministic, explainable CLI and web-based financial decision assistant for equity evaluation using historical stock data (NSE & BSE).
...
RankingExplanationEngine      → ranking results explanation (AI Ranking Insights)
ResponseGenerator             → formatting only — no computation

### Web Backend Architecture
django_webapp                 → Django templates (Tailwind + Chart.js)
django_api                    → HTTP JSON endpoints wrapping core Engines
Services (e.g. RankingService) → Stateful request orchestration, error isolation
Schemas (e.g. RankingResponse) → Strongly typed Pydantic API contracts
...
| Metric | Direction |
|---|---|
| CAGR, Custom Score, Sharpe, Sortino, Avg Volume | `higher_is_better = True` |
| Volatility, Max Drawdown | `higher_is_better = False` (lower = safer = better) |

---

## 5️⃣ Weighted Custom Metric *(Web UI)*

The **Custom** metric (formerly "Score") lets users define their own weighting across all six criteria, both in the **Ranking** and **Portfolio Analysis** pages.

### Available weight criteria

| Criterion | Key | Default Weight |
|---|---|---|
| Return (CAGR) | `weight_return` | 0.34 |
| Risk (Volatility) | `weight_risk` | 0.33 |
| Volume | `weight_volume` | 0.33 |
| Sharpe Ratio | `weight_sharpe` | 0.0 |
| Max Drawdown | `weight_max_drawdown` | 0.0 |
| Sortino Ratio | `weight_sortino` | 0.0 |

**Rules:**
- Weights are **relative** — they are auto-normalised to sum to 100% before scoring.
- A weight of `0` fully excludes a criterion.
- Input supports up to **2 decimal places** (e.g. `0.34`).
- The panel is **hidden by default** and toggled by a button on both pages.

### Exchange-Specific Result Limits

When requesting top-N results, the UI enforces hard caps per exchange:
...
The `max` attribute on the N input updates dynamically when the exchange selection changes.
...
## 9️⃣ Web Application + AI Explanations

The system includes a full Django-powered Web Application (`127.0.0.1:8000`) alongside the CLI.

### Features
- **Interactive Dashboards**: Ranking, Portfolio Optimizer, and Risk Analytics pages with Chart.js visualisations (scatter plots, allocation pies, CAGR bars, Monte Carlo histograms).
- **AI Ranking Insights**: After every ranking query, an `AI Ranking Insights` panel appears showing the Screen Summary, Methodology, Metric Explanation, Top Results table, optional Weights breakdown (Custom metric only), and a Closing Note — generated by `RankingExplanationEngine`.
- **AI Portfolio Interpretation**: Deep integration of `AllocationExplanationEngine`. Generates structured prose (Executive Summary, Strategy Rationale, Risk Distribution, Portfolio Risk metrics, Monte Carlo Results, Final Verdict) into dedicated UI cards.
- **Custom Scoring Weights (Ranking & Portfolio)**: Both pages expose a collapsible Custom Weights panel. When open, six weight inputs (Return, Risk, Volume, Sharpe, Max Drawdown, Sortino) are sent to the backend and used in `ScoringEngine.compute_weighted_scores()`. Falls back to defaults when the panel is closed.
- **Fail-Safe APIs**: Explanation generation is strictly isolated. If the text engine fails, the numerical API still returns `200 OK` with chart/table data intact.
- **Strictly Typed Contracts**: All API endpoints use `Pydantic` schemas to ensure the frontend receives guaranteed struct types.
- **Market-Aware Ticker Picker**: Instantly loads top tickers for the selected Exchange (NSE vs BSE) on focus/click.
- **Exchange-Validated N input**: Max N is enforced client-side (NSE ≤ 1,970 / BSE ≤ 4,465) with a browser validation error if exceeded.
...
* **Explainable** — every recommendation comes with structured breakdowns (CLI: 5-section narrative; Web: dedicated AI Insight cards for both ranking and portfolio)
* **Modular** — 13 single-responsibility engines
* **Production-grade tested** — **110+ tests** covering mathematical invariants, parser stress, behavioral profiles, cache lifecycle, positional queries, metric-direction inversion, sort/filter logic, weighted scoring, and API weight parsing
* **Fast** — screener queries served in < 1 second from fingerprinted cache
* **Risk-aware** — 4 stages of portfolio risk modeling from simple to full Monte Carlo simulation
* **Flexible querying** — list mode (`top N`), positional mode (`worst`, `2nd best`), in-session sort/filter, and user-defined weighted scoring
* **User-Controlled Scoring** — all six financial criteria can be individually weighted on both the Ranking and Portfolio pages
